...
class Paddle(pygame.sprite.Sprite):
    def __init__(self, groups):
        ...
        pygame.draw.rect(self.shadow_surf, COLORS['paddle shadow'], pygame.FRect((0,0), (SIZE['paddle'])), 0, 6)
...
class Opponent(Paddle):
    ...
    def get_direction(self):
        self.direction = 1 if self.ball.rect.centery > self.rect.centery else -1 # I don't know how this works, but it does
...
class Ball(pygame.sprite.Sprite)  :
    def __init__(self, groups, paddle_sprites, update_score):
        ...
        # ball & movement (elite ball knowledge)      
        self.rect = self.image.get_frect(center = (WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2))
        self.old_rect = self.rect.copy()
        self.direction = pygame.Vector2(choice((1,-1)), uniform(0.7, 0.8) * choice((-1, 1))) # man, I don't even know myself
    ...
    def collision(self, direction):
        for sprite in self.paddle_sprites:
            if sprite.rect.colliderect(self.rect):  # checking if player and ball collide
                if direction == 'horizontal':
                    ...
                    if self.rect.bottom >= sprite.rect.top and self.old_rect.bottom <= sprite.old_rect.top: # checks if ball is top of paddle
